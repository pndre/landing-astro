...
function getFilteredAndSorted() {
    let filtered = loadedImages.filter(img => {
        const matchCategory = currentCategory === "Todos" || img.tag === currentCategory;
...
function renderGallery() {
    ...
    statsSpan.innerText = `${itemsToShow.length} de ${loadedImages.length} imagens`;
...
function updateModalContent() {
    ...
    downloadBtn.onclick = () => {
        fetch(imgData.src)
            ...
            .catch(() => alert("Download disponível apenas para imagens de origem."));
    };
}
...
function loadMore() {
    if (currentLimit >= masterImageList.length) {
        alert("🎞️ Coleção completa carregada! Todas as imagens já estão disponíveis.");
        return;
    }
...
document.getElementById("mockNewsletter")?.addEventListener("click", () => alert("📸 Arquivo Visual: obrigado pelo interesse! Novidades em breve na sua caixa de entrada (demonstração)."));
document.querySelectorAll('.social-links i').forEach(icon => {
    icon.addEventListener('click', () => alert('Conecte-se com o Arquivo Visual nas redes sociais (demonstração interativa).'));
});

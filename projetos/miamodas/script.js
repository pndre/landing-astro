const products = [{
        id: 1,
        name: "Vestido Midi Escultural",
        price: 1890,
        category: "dress",
        desc: "Silhueta assimétrica em tecido orgânico.",
    ...
    {
        id: 2,
        name: "Blusa de Seda Atemporal",
        price: 890,
        category: "top",
        desc: "Seda natural fluida.",
    ...
    {
        id: 3,
        name: "Tailleur de Linho Orgânico",
        price: 2450,
        category: "blazer",
        desc: "Conjunto oversized artesanal.",
    ...
    {
        id: 4,
        name: "Vestido Longo para Cerimônia",
        price: 3200,
        category: "dress",
        desc: "Drapping escultural.",
    ...
    {
        id: 5,
        name: "Camisa Oversize de Algodão",
        price: 650,
        category: "shirt",
        desc: "Botões de madrepérola.",
    ...
    {
        id: 6,
        name: "Vestido de Couro Vegetal",
        price: 2790,
        category: "dress",
        desc: "Sustentável e minimalista.",
    ...
    {
        id: 9,
        name: "Top Cropped de Seda",
        price: 720,
        category: "top",
        desc: "Detalhe franzido.",
...
function toggleWishlist(pid) {
    ...
    if (idx > -1) {
        ...
        showToast(`${prod.name} removido dos favoritos`);
    } else {
        wishlist.push(prod);
        showToast(`${prod.name} adicionado aos favoritos ✨`);
    }
    save();
    updateCounters();
    renderGallery();
...
function addToCart(prod, size, qty = 1) {
    if (!size) {
        showToast("Por favor, escolha um tamanho");
        return false;
    }
    ...
    save();
    showToast(`${prod.name} (${size}) adicionado`);
    renderCartDrawer();
    return true;
}
...
function renderCartDrawer() {
    ...
    if (cart.length === 0) {
        container.innerHTML = "<p style='text-align:center;padding:2rem;'>Sua curadoria está vazia</p>";
    ...
    container.innerHTML = cart.map((item, i) => {
        ...
        total += subtotal;
        return `<div class="cart-item"><img class="cart-item-img" src="${p.images[0]}"><div class="cart-item-info"><div class="cart-item-name">${p.name}</div><div>Tamanho: ${item.size}</div><div class="cart-item-price">R$ ${p.price}</div><div class="quantity-control"><button class="cart-dec" data-index="${i}">-</button><span>${item.qty}</span><button class="cart-inc" data-index="${i}">+</button><button class="cart-remove" data-index="${i}" style="margin-left:8px;">🗑️</button></div></div></div>`;
    }).join("");
...
function openDetail(id) {
    ...
    document.getElementById("reviewsList").innerHTML = selectedProduct.reviews.map(r => `<div style="border-bottom:1px solid #eae6e0; padding:1rem 0;"><strong>${r.user}</strong><p>${r.text}</p></div>`).join("") || "<p>Nenhuma avaliação ainda.</p>";
...
function openCheckout() {
    ...
    document.getElementById("checkoutSummary").innerHTML = `Resumo: ${cart.length} itens | Total R$ ${total.toFixed(2)}`;
}
document.getElementById("checkoutForm").addEventListener("submit", (e) => {
    ...
    showToast(`✨ Pedido concluído! Total: R$ ${final.toFixed(2)}. Obrigado!`);
    cart = [];
    save();
    renderCartDrawer();
    updateCounters();
...
function renderWishlistModal() {
    ...
    if (wishlist.length === 0) container.innerHTML = "<p>Lista de desejos vazia</p>";
}
...
track.innerHTML = craftItems.map(src => `<div class="carousel-item"><img src="${src}"><p style="margin-top:0.5rem; font-size:0.7rem;">Etapa artesanal</p></div>`).join("");
...
function initFilters() {
    ...
    container.innerHTML = cats.map(c => `<button class="filter-btn ${c===currentFilter?"active":""}" data-cat="${c}">${catNames[c]}</button>`).join("");
...
document.getElementById("navAtelier").addEventListener("click", e => {
    e.preventDefault();
    showToast("Ateliê exclusivo na Bahia — cada peça feita sob encomenda.");
});
document.getElementById("navContato").addEventListener("click", e => {
    e.preventDefault();
    showToast("hello@miamodas.com | WhatsApp (71) 99123-4567");
});
